            ...
            ->each(function ($node) {
                ...
                if ($date->format('j.n.Y') === (new \DateTime())->format('j.n.Y')) {
                    ...
                    foreach ($node->filter('div') as $tmp) {
                        $key = strpos($tmp->parentNode->getAttribute('class'), 'polevka') === false ? static::KEY_MAIN : static::KEY_SOUPS;

                        if (strpos($tmp->getAttribute('class'), 'polozka') !== false) {
